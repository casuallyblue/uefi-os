...
fn get_output_path() -> PathBuf {
    ...
    let path = Path::new(&manifest_dir_string).join("target/x86_64-unknown-uefi").join(build_type);
    path
}

fn main() {
    println!("cargo:rerun-if-changed=boot/startup.nsh");
    let startup_nsh_path = Path::new(&env::var("CARGO_MANIFEST_DIR").unwrap()).join("boot").join("startup.nsh");
